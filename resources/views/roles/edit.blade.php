...
<style>
    ...
    .permission-check { display: flex; align-items: center; padding: 12px 16px; background: #f9fafb; border: 2px solid #e5e7eb; border-radius: 10px; cursor: pointer; transition: all 0.3s ease; }
    .permission-check:hover { border-color: #0d3b66; background: rgba(13, 59, 102, 0.05); }
    .permission-check input[type="checkbox"] { width: 20px; height: 20px; margin-right: 12px; cursor: pointer; accent-color: #0d3b66; }
    .permission-check input[type="checkbox"]:checked { accent-color: #10b981; }
    ...
    @media (max-width: 768px) { .form-header { padding: 30px 20px; } .form-container > form { padding: 30px 20px; } .permissions-grid { grid-template-columns: 1fr; } .form-actions { flex-direction: column; } .btn { width: 100%; flex: none; } }
</style>
    ...
    <div class="form-container">
        ...
        <form method="POST" action="{{ route('roles.update', $role->id) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" name="name" id="name" class="form-input @error('name') error @enderror" value="{{ old('name', $role->name) }}" placeholder="Enter role name">
                ...
                <div class="permissions-grid">
                    @foreach($permission as $value)
                        <label class="permission-check">
                            <input type="checkbox" name="permission[]" value="{{$value->id}}" {{ in_array($value->id, old('permission', $rolePermissions)) ? 'checked' : '' }}>
                            <span class="permission-label">{{ ucfirst($value->name) }}</span>
                        </label>
                    @endforeach
                </div>
