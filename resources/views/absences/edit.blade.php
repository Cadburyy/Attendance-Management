...
<style>
    ...
    .error-list li { font-size: 14px; padding: 2px 0; }
...
<div class="form-wrapper">
    <div class="form-container">
        <div class="form-header">
            <h1 class="form-title">Edit Absence Request</h1>
            <p class="form-subtitle">Update the absence request details</p>
        </div>
        ...
        <form method="POST" action="{{ route('absences.update', $absence) }}">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-input @error('employee_name') error @enderror" 
                    id="employee_name" name="employee_name" value="{{ old('employee_name', $absence->employee_name) }}" required>
                @error('employee_name')<span class="error-text">{{ $message }}</span>@enderror
            </div>
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="date" class="form-input @error('date') error @enderror" 
                        id="date" name="date" value="{{ old('date', $absence->date->format('Y-m-d')) }}" required>
                ...
                <div class="form-group">
                    ...
                    <select class="form-input @error('reason') error @enderror" id="reason" name="reason" required>
                        ...
                        <option value="sick" {{ old('reason', $absence->reason) == 'sick' ? 'selected' : '' }}>Sick</option>
                        <option value="leave" {{ old('reason', $absence->reason) == 'leave' ? 'selected' : '' }}>Casual Leave</option>
                        <option value="emergency" {{ old('reason', $absence->reason) == 'emergency' ? 'selected' : '' }}>Emergency</option>
                        <option value="other" {{ old('reason', $absence->reason) == 'other' ? 'selected' : '' }}>Other</option>
                    </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-input textarea @error('details') error @enderror" 
                    id="details" name="details" rows="5">{{ old('details', $absence->details) }}</textarea>
                @error('details')<span class="error-text">{{ $message }}</span>@enderror
            </div>

            <div class="form-actions">
                <button type="submit" class="btn btn-submit">
                    <i class="fas fa-check"></i> Update Request
                </button>
